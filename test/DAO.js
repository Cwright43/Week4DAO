...
describe('DAO', () => {
  ...
  beforeEach(async () => {
    ...
    // Send tokens to investor - each one gets 20%
    ...
    // Deploy DAO - getContractFactory('DAO')
    // Let's say quorum is 51% of all tokens, so about 500,000
    // Therefore, we will code for 500,000 + 1 WEI

    const DAO = await ethers.getContractFactory('DAO')
    dao = await DAO.deploy(token.address, '500000000000000000000001')

    // Funder sends 100 Ether in DAO treasury for governance
  ...
  describe('Proposal creation', () => {
    ...
    describe('Failure', () => {
      it('rejects invalid amount', async () => {
        ...
        // Contract only has 100 ETH, and so this will NOT WORK at 1000 ETH
...
describe('Governance', () => {
    ...
    describe('Success', () => {
      ...
      it('updates the proposal to finalized state', async () => {
          const proposal = await dao.proposals(1)
    ...
    describe('Failure', () => {
      ...
      it('rejects proposal if already finalized', async () => {
        ...
        result = await transaction.wait()
         ...
         // Try to finalize a 2ND time - we want this to be reverted
         transaction = await expect(dao.connect(investor1).finalizeProposal(1)).to.be.reverted
      })
